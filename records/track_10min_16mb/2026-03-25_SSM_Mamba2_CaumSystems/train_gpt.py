"""
First State-Space Model (SSM) submission to OpenAI Parameter Golf.
Architecture: Mamba-2 SSD blocks with U-Net skip connections.
No attention mechanism — pure SSM sequence modeling.

CAUM Systems — https://github.com/Blasmerit
"""
...
# Try to import optimized Mamba-2 (CUDA kernels). Fall back to pure PyTorch.
try:
...
# - 10 Mamba-2 blocks at width 512 (no separate MLP — Mamba includes 2x expansion)
# - d_state=64, d_conv=4, headdim=64 (Mamba-2 defaults)
# - U-Net skip connections (same as baseline transformer)
# - ~17.1M params → ~12MB after int8+zlib → fits in 16MB
# - Per-block: ~1.65M params (6*D^2 vs 12*D^2 for transformer+MLP)
...
# Tensor name patterns for control parameters (kept in fp32, not quantized to int8).
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    p for p in os.environ.get(
        "CONTROL_TENSOR_NAME_PATTERNS",
        "block_scale,skip_weight,skip_weights,A_log,D,dt_bias",
    ).split(",") if p
)
...
# -----------------------------
# NAIVE SSM FALLBACK (pure PyTorch, no custom CUDA kernels)
# For local testing only. VERY SLOW — use mamba_ssm.Mamba2 for real training.
# -----------------------------

class NaiveSSM(nn.Module):
    """Minimal Mamba-style SSM in pure PyTorch. Sequential scan — O(L) per step."""
    ...
    def __init__(self, d_model: int, d_state: int = 16, d_conv: int = 4, expand: int = 2):
        ...
        # SSM parameter projections (B, C from input; dt from input).
...
# -----------------------------
# MODEL — Mamba-2 SSM with U-Net Skips
# -----------------------------
...
class SSMBlock(nn.Module):
    """Single Mamba-2 block with norm, residual, and learned scale."""
    ...
    def __init__(self, d_model: int, d_state: int, d_conv: int, expand: int, headdim: int):
        ...
        if HAS_MAMBA_SSM:
            ...
            print("WARNING: mamba_ssm not found. Using NaiveSSM (very slow).")
    ...
    def forward(self, x: Tensor, x0: Tensor) -> Tensor:
        # Residual mix with original embedding (U-Net style).
        normed = self.norm(x)
        ssm_out = self.ssm(normed)
        return x + self.block_scale.to(dtype=x.dtype)[None, None, :] * ssm_out
...
class MambaLM(nn.Module):
    ...
    def __init__(
        ...
        d_state: int, d_conv: int, expand: int, headdim: int,
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Encoder: first half stores skip connections.
        for i in range(self.num_encoder_layers):
            x = self.blocks[i](x, x0)
            skips.append(x)
        # Decoder: second half consumes skips in reverse.
...
def main() -> None:
    ...
    log0(f"mamba_ssm available: {HAS_MAMBA_SSM}", console=False)
    log0(
    ...
    # Restore control parameters to fp32.
    with torch.no_grad():
        for name, param in base_model.named_parameters():
            if (param.ndim < 2 or any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS)) and param.dtype != torch.float32:
                param.data = param.data.float()

    # torch.compile — use fullgraph=False for compatibility with Mamba-2 custom ops.
    use_compile = bool(int(os.environ.get("USE_COMPILE", "1" if HAS_MAMBA_SSM else "0")))
    if use_compile:
    ...
    # Optimizer split: Muon for 2D matrix params, Adam for scalars/embeddings.
    ...
    log0(f"Architecture: Mamba-2 SSM ({args.num_layers} layers, d_model={args.model_dim}, "
         f"d_state={args.d_state}, expand={args.expand}, headdim={args.headdim})")
    log0(f"model_params:{n_params}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(f"mamba_ssm:{HAS_MAMBA_SSM} torch_compile:{use_compile}")
