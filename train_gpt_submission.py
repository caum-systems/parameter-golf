...
class Hyperparameters:
    ...
    inception_enabled = bool(int(os.environ.get("INCEPTION_ENABLED", "1")))
...
class SVDEmbedding(nn.Module):
    """Low-rank factored embedding: E[token] = (U[token] * S) @ V.
    Saves ~87% params for vocab=16384 rank=64 vs full embedding."""
    def __init__(self, vocab_size: int, dim: int, rank: int = 64, init_std: float = 0.02):
        super().__init__()
    ...
    def forward(self, input_ids: Tensor) -> Tensor:
        u = self.U[input_ids]
        us = u * self.S.to(dtype=u.dtype)
        return us @ self.V
    @property
    def weight(self) -> Tensor:
        """Reconstruct full [vocab, dim] matrix for tied embedding logit projection."""
        US = self.U * self.S.to(dtype=self.U.dtype).unsqueeze(0)
        return US @ self.V

# ---- TrigramHash Logit Bias + Mixture of Softmaxes ----

class TrigramLogitBias(nn.Module):
    """Learned n-gram -> logit bias via hash table + low-rank projection."""
    def __init__(self, vocab_size: int = 8192, proj_dim: int = 16,
                 table_size: int = 4096, max_order: int = 4, min_order: int = 2,
        ...
        self.tables = nn.Parameter(torch.randn(num_orders, table_size, proj_dim) * init_scale)
        self.proj = nn.Linear(proj_dim, vocab_size, bias=False)
        nn.init.normal_(self.proj.weight, std=init_scale)
        self.order_scales = nn.Parameter(torch.full((num_orders,), 0.1))
        primes = [1, 31, 997, 30011, 900001]
        self.register_buffer('hash_primes', torch.tensor(primes[:max_order], dtype=torch.long))

    def _hash_ngram(self, token_ids: Tensor, order: int) -> Tensor:
        B, T = token_ids.shape
    ...
    def forward(self, input_ids: Tensor) -> Tensor:
        ...
        for i, order in enumerate(range(self.min_order, self.max_order + 1)):
            indices = self._hash_ngram(input_ids, order)
            table_vectors = self.tables[i][indices]
            total_bias = total_bias + table_vectors * self.order_scales[i]
        return self.proj(total_bias)

class MixtureOfSoftmaxes(nn.Module):
    """K mixed softmaxes breaking the softmax bottleneck (Yang et al. 2018)."""
    def __init__(self, dim: int, vocab_size: int, num_experts: int = 4,
        ...
        self.expert_temps = nn.Parameter(torch.ones(num_experts))

    def forward(self, hidden: Tensor, embedding_weight: Tensor,
                extra_logit_bias: Tensor = None) -> Tensor:
        K = self.num_experts
        pi = F.softmax(self.gate(hidden), dim=-1)
        expert_logits_list = []
        for k in range(K):
            h_k = hidden * self.expert_scales[k].unsqueeze(0)
            logits_k = F.linear(h_k, embedding_weight)
        ...
        log_pi = torch.log(pi.T.unsqueeze(-1) + 1e-8)
        log_softmax_k = all_logits - all_logits.logsumexp(dim=-1, keepdim=True)
        return torch.logsumexp(log_pi + log_softmax_k, dim=0)

class RefinementAttention(nn.Module):
    """Minimal causal self-attention for the refinement head."""
...
class RefinementMLP(nn.Module):
    """Minimal MLP for refinement head."""
...
class InceptionRefinementHead(nn.Module):
    """Mini-transformer that refines hidden states before logit projection.
    All output projections zero-init so at init this is the identity function."""
    def __init__(self, main_dim: int = 512, refine_dim: int = 64,
                 num_layers: int = 1, num_heads: int = 2, mlp_mult: int = 2,
                 residual_scale: float = 0.1):
        super().__init__()
        self.proj_down = nn.Linear(main_dim, refine_dim, bias=False)
        self.layers = nn.ModuleList()
        for _ in range(num_layers):
            self.layers.append(nn.ModuleDict({
                'norm1': nn.LayerNorm(refine_dim),
                'attn': RefinementAttention(refine_dim, num_heads),
                'norm2': nn.LayerNorm(refine_dim),
                'mlp': RefinementMLP(refine_dim, mlp_mult),
            }))
        self.final_norm = nn.LayerNorm(refine_dim)
        self.proj_up = nn.Linear(refine_dim, main_dim, bias=False)
...
class EnhancedOutputHead(nn.Module):
    ...
    def __init__(self, dim: int, vocab_size: int, logit_softcap: float = 30.0,
        ...
        self.vocab_size = vocab_size
        if use_inception:
            self.inception = InceptionRefinementHead(
                main_dim=dim, refine_dim=inception_dim, num_layers=inception_layers)
        if use_trigram:
...
class GPT(nn.Module):
    def __init__(
        ...
        if svd_rank > 0 and vocab_size > 4096:
            self.tok_emb = SVDEmbedding(vocab_size, model_dim, rank=svd_rank, init_std=tied_embed_init_std)
        else:
        ...
        self.output_head = EnhancedOutputHead(
            ...
            trigram_proj_dim=trigram_proj_dim, trigram_max_order=trigram_max_order,
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.output_head is not None:
            main_loss = self.output_head(x, input_ids, target_ids, self.tok_emb.weight)
        else:
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        x = self.final_norm(x)
        if self.output_head is not None:
            return self.output_head.get_log_probs(x, input_ids, self.tok_emb.weight)
        if self.tie_embeddings:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mos_enabled=args.mos_enabled,
        mos_experts=args.mos_experts,
        trigram_enabled=args.trigram_enabled,
        trigram_table_size=args.trigram_table_size,
        trigram_proj_dim=args.trigram_proj_dim,
        trigram_max_order=args.trigram_max_order,
        inception_enabled=args.inception_enabled,
    ...
    if isinstance(base_model.tok_emb, SVDEmbedding):
        tok_params = [{"params": [base_model.tok_emb.U, base_model.tok_emb.V], "lr": token_lr, "base_lr": token_lr}]
        scalar_params.append(base_model.tok_emb.S)
    else:
    ...
    if base_model.output_head is not None:
        for p in base_model.output_head.parameters():
