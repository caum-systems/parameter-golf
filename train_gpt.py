...
class Hyperparameters:
    ...
    svd_rank = int(os.environ.get("SVD_RANK", 32))
...
class SVDEmbedding(nn.Module):
    """Low-rank embedding: E(x) = U[x] * S @ V. Saves (V-r)(d-r) params vs nn.Embedding."""
    def __init__(self, num_embeddings: int, embedding_dim: int, rank: int):
        super().__init__()
        self.num_embeddings = num_embeddings
        self.embedding_dim = embedding_dim
        self.rank = rank
        self.U = nn.Parameter(torch.randn(num_embeddings, rank) * 0.01)
        self.S = nn.Parameter(torch.ones(rank))
        self.V = nn.Parameter(torch.randn(rank, embedding_dim) * 0.01)
        nn.init.orthogonal_(self.V)
    def forward(self, input_ids: Tensor) -> Tensor:
        return (F.embedding(input_ids, self.U) * self.S.to(dtype=self.U.dtype)) @ self.V.to(dtype=self.U.dtype)
    @property
    def weight(self) -> Tensor:
        """Full weight matrix for tied output head: (V, d) = (U * S) @ V."""
        return (self.U.to(dtype=self.V.dtype) * self.S.to(dtype=self.V.dtype)) @ self.V
...
class GPT(nn.Module):
    def __init__(
        ...
        if svd_rank > 0:
            self.tok_emb = SVDEmbedding(vocab_size, model_dim, svd_rank)
        else:
    ...
    def _init_weights(self) -> None:
        if self.tie_embeddings and self.svd_rank == 0:
...
def main() -> None:
    ...
    if args.svd_rank > 0:
        tok_params = [
            {"params": [base_model.tok_emb.U], "lr": token_lr, "base_lr": token_lr},
            {"params": [base_model.tok_emb.V], "lr": token_lr, "base_lr": token_lr},
        ]
